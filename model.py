...
class GemmaForCausalLM(nn.Module):
    ...
    def generate(
        ...
        attention_weights = {}
        for i, decoders in enumerate(self.model.layers):
            attention_weights[f"GemmaDecoderLayer-{i+1}"] = []
        for i in range(max_seq_len - min_prompt_len):
            ...
            for i, decoders in enumerate(self.model.layers):
                attention_weights[f"GemmaDecoderLayer-{i+1}"].append(decoders.self_attn.attention_weight.detach().cpu().tolist())
        ...
        return (results[0],token_ids_tensor, attention_weights) if is_str_prompt else (results,token_ids_tensor, attention_weights)
